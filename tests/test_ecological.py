...
def test_ecological_detector_uses_joint_sample_and_primary_f_ratio() -> None:
    ...
    expected_f = (
        comparison.factor_1_result.within_ss
        / comparison.factor_2_result.within_ss
    )
...
def test_more_explanatory_factor_has_lower_within_ss_when_significant() -> None:
    ...
    comparison = EcologicalDetector(alpha=0.2).fit(
        y, {"strong": strong, "weak": weak}
    ).comparisons[0]
...
def test_greater_compatibility_is_order_dependent_by_design() -> None:
    ...
    forward = EcologicalDetector(alternative="greater").fit(
        y, {"strong": strong, "weak": weak}
    ).comparisons[0]
    reverse = EcologicalDetector(alternative="greater").fit(
        y, {"weak": weak, "strong": strong}
    ).comparisons[0]
